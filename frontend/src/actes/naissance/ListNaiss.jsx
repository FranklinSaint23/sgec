...
function ListNaiss() {
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/actes_naissance/${acteToDelete.id}`);
  ...
  const rechercherIA = async () => {
    ...
    setResultatsIA(null);
    try {
      const res = await api.get(`/search?type=naissance&q=${encodeURIComponent(searchIA)}`);
      setResultatsIA(res.data);
    } catch {
      alert("Service de recherche IA indisponible.");
    } finally {
      setLoadingIA(false);
    }
  };
  ...
  const colonnes = [ 
    ...
    { name: "Nom de l'enfant", selector: (acte) => acte.nom, sortable: true },
    { name: "Date de naissance", selector: (acte) => acte.date_naiss, sortable: true },
    {
      name: "Sexe",
      selector: (acte) => (acte.sexe === "M" ? "Masculin" : "Féminin"),
      sortable: true,
    },
    { name: "Dressé le", selector: (acte) => acte.dresse, sortable: true },
    {
      name: "Actions",
      cell: (acte) => (
        <div className="d-flex gap-2">
          <Link
            to={`/dashboard?page=edit_naiss&id=${acte.id}`}
          ...
          <Link
            to={`/detail_naiss/${acte.id}`}
            className="btn btn-sm btn-warning"
            title="Détails"
          >
            <i className="bi bi-info-circle"></i>
          </Link>
          <Link to={`/reconnaitre_enfant/${acte.id}`} 
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = "actes_naissance.csv";
    a.click();
  };
  ...
  const exportExcel = () => {
    ...
    XLSX.writeFile(wb, "actes_naissance.xlsx");
  };

  // Export PDF
  const exportPDF = () => {
    const doc = new jsPDF();
    doc.text("Liste des actes de naissance", 14, 10);
    autoTable(doc, {
    ...
    <div className="container mt-4">
      <div className="col-md-12 p-4 bg-light rounded shadow-sm mx-auto">
        <div className="d-flex justify-content-between align-items-center mb-3">
          <h3 className="m-0">📋 Liste des actes de naissance</h3>
          <Link to="/dashboard?page=acte_naissance" className="btn btn-primary">
            <i className="bi bi-plus-circle me-2"></i> Déclarer une naissance
          </Link>
        </div>
        ...
        <div className="input-group mb-3">
          <input
            type="text"
            placeholder="🤖 Recherche approchée (nom, parent…)"
            className="form-control"
            value={searchIA}
            onChange={e => { setSearchIA(e.target.value); setResultatsIA(null); }}
            onKeyDown={e => e.key === "Enter" && rechercherIA()}
          />
          <button className="btn btn-outline-primary" onClick={rechercherIA} disabled={loadingIA}>
            {loadingIA ? "…" : "Recherche IA"}
          </button>
          {resultatsIA && (
            <button className="btn btn-outline-secondary" onClick={() => { setResultatsIA(null); setSearchIA(""); }}>
              Effacer
            </button>
          )}
        </div>
        ...
        {resultatsIA && (
          <div className="alert alert-info py-2 mb-2">
            <strong>{resultatsIA.length} résultat(s) IA</strong> pour « {searchIA} »
          </div>
        )}
