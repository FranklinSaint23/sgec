...
function ListDeces() {
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/actes_deces/${acteToDelete.id}`);
  ...
  const rechercherIA = async () => {
    ...
    try {
      const res = await api.get(`/search?type=deces&q=${encodeURIComponent(searchIA)}`);
  ...
  // Colonnes
  const colonnes = [ 
    { name: "N° Acte", selector: (row) => row.numero_acte, sortable: true },
    { name: "Nom du décédé", selector: (row) => row.nom_decede, sortable: true },
    { name: "Date du décès", selector: (row) => row.date_deces, sortable: true },
    {
      name: "Sexe",
      selector: (row) => (row.sexe === "M" ? "Masculin" : "Féminin"),
      sortable: true,
    },
    { name: "Dressé le", selector: (row) => row.dresse_le, sortable: true },
    {
      name: "Actions",
      cell: (row) => (
        <div className="d-flex gap-2">
          <Link
            to={`/dashboard?page=edit_deces&id=${row.id}`}
  ...
  const exportExcel = () => {
    ...
    XLSX.writeFile(wb, "actes_deces.xlsx");
  };

  // Export PDF
  const exportPDF = () => {
    const doc = new jsPDF();
    doc.text("Liste des actes de décès", 14, 10);
    autoTable(doc, {
      head: [["N° Acte", "Nom", "Date décès", "Sexe", "Dressé le"]],
    ...
    <div className="container mt-4">
      <div className="col-md-12 p-4 bg-light rounded shadow-sm mx-auto">
        <div className="d-flex justify-content-between align-items-center mb-3">
          <h3 className="m-0">📋 Liste des actes de décès</h3>
          <Link to="/dashboard?page=acte_deces" className="btn btn-primary">
            <i className="bi bi-plus-circle me-2"></i> Déclarer un décès
          </Link>
        </div>
        ...
        <div className="input-group mb-3">
          <input type="text" placeholder="🤖 Recherche approchée (nom, date…)" className="form-control"
