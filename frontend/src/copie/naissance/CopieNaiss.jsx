...
function CopieNaiss() {
  ...
  const colonnes = [
    ...
    { name: 'Nom de l\'enfant', selector: (r) => r.nom,         sortable: true },
    { name: 'Date de naissance',selector: (r) => r.date_naiss,  sortable: true },
    { name: 'Sexe',             selector: (r) => r.sexe === 'M' ? 'Masculin' : 'Féminin', sortable: true, width: '110px' },
    { name: 'Dressé le',        selector: (r) => r.dresse,      sortable: true },
    {
      name: 'Actions', width: '110px', ignoreRowClick: true,
      cell: (r) => (
        <Link to={`/detail_copie_naiss/${r.id}`} className="btn btn-sm btn-warning">
    ...
    <div className="ea-table-page">
      <div className="ea-table-page-header">
        <div className="ea-table-page-title">
          <i className="bi bi-file-earmark-person text-success"></i>
          Duplicata — Actes de naissance
          <span className="ea-badge ea-badge-green ms-1">{actes.length}</span>
        </div>
        <input
          type="text"
      ...
      <DataTable
        ...
        progressComponent={<div className="py-5 text-center"><span className="spinner-border text-success"></span></div>}
        noDataComponent={<div className="py-5 text-center text-muted small">Aucun acte de naissance trouvé.</div>}
