...
function CopieDeces() {
  ...
  const colonnes = [
    ...
    { name: 'Nom du décédé', selector: (r) => r.nom_decede,   sortable: true },
    { name: 'Date du décès', selector: (r) => r.date_deces,   sortable: true },
    { name: 'Sexe',          selector: (r) => r.sexe === 'M' ? 'Masculin' : 'Féminin', sortable: true, width: '110px' },
    { name: 'Dressé le',     selector: (r) => r.dresse_le,    sortable: true },
    {
      name: 'Actions', width: '110px', ignoreRowClick: true,
      cell: (r) => (
        <Link to={`/detail_copie_deces/${r.id}`} className="btn btn-sm btn-warning">
    ...
    <div className="ea-table-page">
      <div className="ea-table-page-header">
        <div className="ea-table-page-title">
          <i className="bi bi-file-earmark-x text-danger"></i>
          Duplicata — Actes de décès
          <span className="ea-badge ea-badge-red ms-1">{actes.length}</span>
        </div>
        <input
          type="text"
      ...
      <DataTable
        ...
        progressComponent={<div className="py-5 text-center"><span className="spinner-border text-danger"></span></div>}
        noDataComponent={<div className="py-5 text-center text-muted small">Aucun acte de décès trouvé.</div>}
